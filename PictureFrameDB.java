...
public class PictureFrameDB extends JPanel{
    ...
    public void setFramePhoto(PhotoDB photo){
        photoLabel.setIcon(new ImageIcon(photo.getImage()));
    ...
    //setting the picture only
    public void previewPhoto(ImageIcon image){
        if(image != null)
    ...
    public int getImageIndexInput(){
        if(imageIndex.getText().matches("[-+]?\\d*\\.?\\d+"))
            return Integer.parseInt(imageIndex.getText());
        else
            return -100;
    }
